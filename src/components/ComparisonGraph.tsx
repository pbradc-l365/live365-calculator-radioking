...
export const ComparisonGraph: React.FC<ComparisonGraphProps> = ({
  ...
  // Calculate total RadioKing cost (Base $24 + checked add-on expenses)
  const additionalCost = activeOptions.reduce((sum, opt) => sum + opt.cost, 0);
  const radioKingTotal = radioKingBasePrice + additionalCost;

  const monthlySavings = Math.max(0, radioKingTotal - live365Price);
  const annualSavings = monthlySavings * 12;

  // Max value for scale calculation
  const maxScale = Math.max(500, radioKingTotal * 1.15, live365Price * 1.5);
    ...
    <div className="w-full bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl text-slate-100 flex flex-col justify-between">
      ...
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Live365 flat rate vs. RadioKing stacked expenses
            </p>
          </div>
        ...
        <div className="relative pt-8 pb-4 flex items-end justify-center gap-8 sm:gap-16 min-h-[420px]">
          ...
          {/* Bar 2: RadioKing (Interactive Stacked Bar starting at $24/mo + selectable options) */}
          <div className="w-36 sm:w-44 flex flex-col items-center">
            {/* Price badge above bar */}
            <div className="mb-2 text-center">
              <motion.span
            ...
            <div
              ...
              style={{
                height: `${Math.max(120, (radioKingTotal / maxScale) * maxHeightPx)}px`,
              }}
            >
              {/* 1. Base Starting RadioKing Tier ($24/mo) */}
              <div
                className="w-full bg-slate-800 text-white flex flex-col justify-center px-2 py-1.5 border-b border-slate-700 text-center shrink-0 transition-all"
                style={{
                  height: `${Math.max(48, (radioKingBasePrice / maxScale) * maxHeightPx)}px`,
                }}
              >
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-300 leading-tight">
                  RadioKing Base
                </span>
                <span className="font-mono font-bold text-xs text-white leading-tight">
                  ${radioKingBasePrice.toFixed(2)}
                </span>
              </div>
            ...
            {/* Label below bar */}
            <div className="mt-3 text-center">
              <span className="font-bold text-sm text-white block">RadioKing Stack</span>
